...
The deployed artefact is [`S2_QAT_INT8_r416.tflite`](S2_QAT_INT8_r416.tflite) (3.23 MB).
...
2. Open [`quantize_yolov8n_final_graphs.ipynb`](quantize_yolov8n_final_graphs.ipynb) and run top-to-bottom — it produces every artefact (S0–S4), every plot, and the final TFLite model.

---

## Future Work

- **Structured pruning** — Channel-level pruning would let TFLite shrink convolution shapes for real on-device speed gains, unlike unstructured sparsity which TFLite stores densely.
- **Crack severity grading** — Use D455 depth to estimate physical crack width and classify severity (hairline / minor / major).
- **3D coordinates of crack** — Use SLAM to calculate real-world 3D positions of cracks.
- **Completely automate system** — Develop an autonomous bot with no human intervention.
---
